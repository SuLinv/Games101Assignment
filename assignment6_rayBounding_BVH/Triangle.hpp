...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    // det趋近于0，ray与面接近平行，没有交点
    if (fabs(det) < EPSILON)
        return inter;

    double det_inv = 1. / det;  // 1 / E1·S1
    Vector3f tvec = ray.origin - v0;    // S = O - P0
    u = dotProduct(tvec, pvec) * det_inv;   // b1(u) = (S1·S)*(1/E1·S1)
    if (u < 0 || u > 1) // u∈[0,1]
        return inter;
    Vector3f qvec = crossProduct(tvec, e1); // S2 = S × E1
    v = dotProduct(ray.direction, qvec) * det_inv;  // b2(v) = (S2·D)*(1/E1·S1)
    if (v < 0 || u + v > 1) // v∈[0,1] v+u∈[0,1]
        return inter;
    t_tmp = dotProduct(e2, qvec) * det_inv; // t = (S2·E2)*(1/E1·S1)
    if(t_tmp < 0)   // ray是射线，t>0
        return inter;

    // TODO find ray triangle intersection
    inter.happened = true;  // 有交点
    inter.coords = ray(t_tmp);  // 返回交点坐标Ray::operator()
    inter.normal = normal;  // 法向量
    inter.distance = t_tmp; // double distance
    inter.obj = this;   // this是所有成员函数的隐藏函数，一个const指针，指向当前对象（正在使用的对象）
    inter.m = m;    // class Material 材质
